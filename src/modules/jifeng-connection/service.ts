...
function getAuthorizationDomain(baseUrl: string) {
  const configured = process.env.JIFENG_DOMAIN?.trim();
  if (!configured) return new URL(baseUrl).hostname;
  if (!/^[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,62})$/.test(configured)) {
    connectionError("DEVELOPER_CONFIG_INVALID", "极风服务器配置不完整");
  }
  return configured;
}
...
export async function authorizeJifengConnection(input: {
  ...
  try {
    const authorization = await port.authorize({
      ...
      domain: getAuthorizationDomain(developer.baseUrl),
      email: input.email,
