        ...
        <div className="hidden md:block">
          <ResponsiveDataTable>
            <Table>
              ...
              <TableBody>{orders.length ? orders.map((order) => <TableRow key={order.id}><TableCell><p className="font-semibold text-ink">{order.orderNumber}</p><p className="mt-1 text-xs text-muted">{dateTime(order.createdAt)}</p></TableCell><TableCell><p>{order.customerCode} · {order.customerName}</p><p className="mt-1 text-xs text-muted">{order.storeName}</p></TableCell><TableCell><Badge className={badgeClass(order.status)} variant="secondary">{statuses.find((status) => status.value === order.status)?.label}</Badge>{order.lockExpiresAt ? <p className="mt-1 text-xs text-muted">锁定至 {dateTime(order.lockExpiresAt)}</p> : null}</TableCell><TableCell>{order.totalPackageCount} 包 / {order.totalQuantity} 件</TableCell><TableCell className="text-right font-semibold tabular-nums">{money(order.totalAmountFen)}</TableCell><TableCell>{canCancel(order.status) ? <AdminOrderCancel orderId={order.id} /> : <span className="text-sm text-muted">—</span>}</TableCell></TableRow>) : <TableRow><TableCell className="h-28 text-center text-muted" colSpan={6}>没有符合条件的拿货单。</TableCell></TableRow>}</TableBody>
            </Table>
          </ResponsiveDataTable>
        </div>
        <div className="divide-y divide-border md:hidden">
          {orders.length ? orders.map((order) => <article className="space-y-3 p-4" key={order.id}><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-ink">{order.orderNumber}</p><p className="mt-1 text-xs text-muted">{dateTime(order.createdAt)}</p></div><Badge className={badgeClass(order.status)} variant="secondary">{statuses.find((status) => status.value === order.status)?.label}</Badge></div><div className="grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs text-muted">客户 / 店铺</p><p className="mt-1 text-ink">{order.customerCode} · {order.storeName}</p></div><div><p className="text-xs text-muted">数量 / 金额</p><p className="mt-1 font-semibold text-ink">{order.totalQuantity} 件 · {money(order.totalAmountFen)}</p></div></div>{order.lockExpiresAt ? <p className="text-xs text-muted">库存锁定至 {dateTime(order.lockExpiresAt)}</p> : null}{canCancel(order.status) ? <AdminOrderCancel orderId={order.id} /> : null}</article>) : <div className="p-10 text-center text-sm text-muted">没有符合条件的拿货单。</div>}
        </div>
      </section>
    </div>
  );
