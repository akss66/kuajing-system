...
export default async function CustomersPage() {
  ...
  const totalStores = rows.reduce((sum, row) => sum + row.storeCount, 0);
  ...
  return (
    <div className="space-y-6">
      <PageHeading
        ...
        description="管理固定合作客户、唯一客户账号，以及其名下的多家 TEMU 店铺。"
        title="客户与店铺"
      />

      <MetricStrip
        items={[
          { label: "客户数", value: `${rows.length}` },
          { label: "活跃客户", value: `${activeCustomers}` },
          { label: "店铺总数", value: `${totalStores}` },
          {
            label: "账号正常",
            value: `${rows.filter((row) => row.accountStatus === "ACTIVE").length}`,
          },
        ]}
      />
      ...
      <section className="overflow-hidden rounded-[var(--radius-surface)] border border-border bg-background">
        <DataWorkspaceToolbar
          description="每一张拿货单都必须归属于具体客户与店铺；账号状态和店铺覆盖范围在这里集中查看。"
          title="合作客户"
        />
        <ResponsiveDataTable>
          <Table>
