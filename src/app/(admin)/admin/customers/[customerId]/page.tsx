    ...
    >
      ...
      <label className="space-y-2 text-sm font-medium text-ink">
        ...
        <Input
          className="min-h-11"
          name="reason"
          placeholder="例如：合作暂停 / 恢复发货"
          required
        />
      </label>
      <DangerHint tone="muted">
        当前状态：{statusLabel(status)}。停用不会删除客户与店铺数据。
      </DangerHint>
    </ConfirmedActionForm>
  );
}
  ...
  return (
    <div className="space-y-6">
      <PageHeading
        ...
        description={`${detail.customer.code} · 管理客户资料、唯一账号摘要，以及名下多家店铺的启停与资料维护。`}
        title="客户详情"
      />

      <MetricStrip
        items={[
          { label: "客户状态", value: statusLabel(detail.customer.status) },
          { label: "店铺数量", value: `${detail.stores.length}` },
          { label: "客户账号", value: detail.account ? "已绑定" : "待排查" },
          { label: "联系微信", value: detail.customer.contactWechat || "暂无记录" },
        ]}
      />
          ...
          <section className="rounded-[var(--radius-surface)] border border-border bg-background p-4 sm:p-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">
                  每位客户仅绑定一个登录账号，这里只提供摘要，不新增第二个入口。
                </p>
              </div>
              {detail.account ? (
              ...
              <div className="mt-4 space-y-2 rounded-lg border border-border bg-surface px-4 py-4">
                <label className="space-y-2 text-sm font-medium text-ink">
                  ...
                  <Input
                    ...
                    defaultValue={detail.account.displayName}
                    readOnly
                  />
                </label>
                <label className="space-y-2 text-sm font-medium text-ink">
                  ...
                  <Input
                    className="min-h-11 bg-background"
                    defaultValue={detail.account.email}
                    readOnly
                  />
                </label>
      ...
      <section className="rounded-[var(--radius-surface)] border border-border bg-background p-4 sm:p-5">
        ...
        <p className="mt-1 text-sm text-muted">
          支持同一客户新增多家 TEMU 店铺，并保留各店铺独立的启停状态。
        </p>
        <ActionForm
      ...
      <section className="space-y-4 rounded-[var(--radius-surface)] border border-border bg-background p-4 sm:p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            桌面端保留高密度直接编辑，移动端改为摘要卡折叠展开，避免长表单直出。
          </p>
        </div>
