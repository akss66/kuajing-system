    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-2 text-sm text-muted">
          按客户、店铺、状态和时间筛选统一结算批次。管理员只能整批确认或整批拒绝，不提供单店部分核款。
        </p>
      </header>
      ...
      <section className="overflow-hidden rounded-[var(--radius-surface)] border border-border bg-background">
        <div className="border-b border-border px-4 py-4 sm:px-5">
          ...
          <p className="mt-1 text-sm text-muted">当前条件共 {batches.length} 条。</p>
        </div>
            ...
            <Table>
              ...
              <TableBody>
                {batches.length ? batches.map((batch) => (
                  ...
                  <TableRow>
                    <TableCell className="h-28 text-center text-muted" colSpan={6}>没有符合条件的结算批次。</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
