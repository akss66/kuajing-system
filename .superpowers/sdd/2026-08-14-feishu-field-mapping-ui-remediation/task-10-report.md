...
## Fix Round 2 — URL filter reset synchronization

Task 11 browser RED found that the GET filter form used `defaultValue` without remounting when Next navigation supplied new URL-derived props. After following `重置筛选`, the URL and server props were empty but the existing DOM retained the old movement type, so a later source-only submission accidentally resubmitted `type=MANUAL_DECREASE`.

The focused component RED rendered populated filters, rerendered the same movement view with empty canonical props, and expected SKU/date/type/operator/source controls to clear. It exited `1` with 1 failure and 5 passes because the SKU DOM value remained `TZX-LOW-001`.

The minimal fix keys the native GET form with the existing canonical filter URL generated by `movementHref(filters, 1)`. Any canonical URL-filter change remounts the uncontrolled form as one consistent unit; no duplicate client filter state or synchronization effect was added.

After the fix, the focused command exited `0` with 6/6 tests passed, including all six filter controls resetting to the new props.

## Fix Round 3 — bounded desktop table geometry

Task 11's real browser and visual RED found that the shared table cells default to `whitespace-nowrap` and both inventory tables still used automatic column layout. At 1440px, a long snapshot name expanded inside its cell by hundreds of pixels and covered total/locked/available/action facts; long movement reason, remark, operator, and relation text similarly clipped or covered later columns at desktop widths.

The focused RED required each desktop table to use `table-fixed` with nine explicit semantic columns and required long-content cells to override nowrap with `min-w-0`, `whitespace-normal`, and `overflow-wrap:anywhere`. It exited `1` with 2 failures and 4 passes because neither table had fixed geometry.

The minimal fix adds local `colgroup` percentages totaling 100% to the realtime and movement tables. Snapshot SKU/name and movement SKU/reason+remark/operator/source/relation cells now wrap inside their assigned columns; numeric, time, status, and action cells retain their compact non-wrapping behavior. Mobile card markup and global `Table` primitives were not changed, and no required fact is truncated or hidden.

After the fix, the focused command exited `0` with 6/6 tests passed.
