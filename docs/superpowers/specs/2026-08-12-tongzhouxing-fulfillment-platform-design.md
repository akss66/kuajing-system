...
状态：业务访谈、分段设计和书面规格均已确认通过。

UI 实施必须同时遵守 `docs/superpowers/specs/2026-08-12-tongzhouxing-ui-design-brief.md` 和项目根目录的 `PRODUCT.md`、`DESIGN.md`。功能规格与 UI 简报冲突时，业务规则以本文件为准，视觉和响应式规则以 UI 简报为准。
...
- 平台正式名称为“同舟行跨境”。
- 正式图形标志来源为 `C:\Users\AKSSINA\Desktop\同舟行logo.png`，原图为 683×656 的透明背景 PNG。
- 实施时将原图复制到项目品牌资源目录，保留透明背景和宽高比例，不拉伸、不重绘标志形状。
- 登录页、客户门户、管理后台和浏览器图标使用同一标志；页面文字使用“同舟行跨境”。

### 8.4 UI 完成度与响应式标准

- 所有交付页面必须达到生产级高保真，不接受仅能使用的临时后台模板。
- 第一版只提供浅色主题。深海青绿用于主操作、选中、链接和焦点；Logo 红不用于主操作。
- 全站采用 A“运营台账”的左侧导航骨架；订单、库存和资金页面采用 C“任务指挥栏”的工具栏、筛选、状态页签和批量操作；B“商务工作区”的异常侧栏仅用于运营总览。
- 管理端保持中等偏紧凑密度；客户门户使用相同设计系统但提供更大的触控区域和更宽松的局部间距。
- 移动端必须在 360px、390px、430px 宽度下完成结构性适配和真实浏览器验收，禁止通过缩小桌面页面完成适配。
- 界面达到 WCAG 2.2 AA，并覆盖键盘焦点、减少动画、加载、空数据、错误、成功、禁用、权限拒绝和外部 API 降级状态。
