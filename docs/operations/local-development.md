...
- 每分钟按平台订单号匹配极风已有订单，并处理重试与状态同步。
- 每分钟处理飞书货盘与内部通知队列。
- 每天按多伦多时区 09:00 生成库存覆盖预警。
- 每天 UTC 03:15 清理已超过 30 天保留期的智能 SKU 匹配运行与逐行建议记录。

批量草稿本身不靠后台 worker 清理。草稿与其导入文件在领域服务读写时按 24 小时 TTL 判定过期，过期后保留只读诊断，不允许继续上传、删文件或再次提交。

极风匹配使用租约回收策略：`integration_outbox.claim_token` 持有当前处理 owner，`locked_at` 超过租约或异常缺失时允许新 worker 接管。系统只按 TEMU 包裹订单号精确查询并绑定极风已有订单，绝不创建或重建极风订单。数据库中历史事件名 `JIFENG_CREATE_ORDER` 与幂等键前缀 `jifeng:create:` 仅为兼容既有 Outbox 数据，不能据此恢复创建接口。
...
智能 SKU 匹配默认关闭。需要本地联调时，额外配置 `DEEPSEEK_API_KEY`，保持 `DEEPSEEK_MODEL=deepseek-v4-flash`，再显式设置 `AI_SKU_MATCH_ENABLED=true`；随后仍需由超级管理员在账号管理中逐客户开放。密钥仅允许进入服务器环境，不能使用 `NEXT_PUBLIC_` 前缀。
